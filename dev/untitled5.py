# -*- coding: utf-8 -*-
"""
Created on Wed Aug  2 17:04:25 2023

@author: taichi.mitsuhashi
"""

# FASMBC_USD
# Monetary, Central Bank Survey, Monetary Base, Currency In Circulation, US Dollars
...
def get_dict_of_country(country_code):
    ...
    col_df = []
    temp_url = api_url+"IFS/M."+country_code+".FASMBC_USD"
    #print(temp_url)
    ex=""
    try:
...
def create_graph(dic_summary):
    ...
    plt.ylabel("Monetary, Central Bank Survey, Monetary Base, Currency In Circulation, US Dollars")
...
for key, val in dic_df_summary.items():
    ...
    df_temp.to_csv(f"CSV_Store\\data_USD_{country_code}_{str_date}.csv")
    ...
    if country_code!="World":
        ...
        pass
    if country_name!="":
        plt.title(f"Fig of FASMBC_{country_name} : USD")
    else:
    ...
    plt.savefig(f"Image_Store\\fig_USD_{country_code}_{str_date}.png")
    plt.show()
