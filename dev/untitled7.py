# -*- coding: utf-8 -*-
"""
Created on Thu Aug  3 10:56:04 2023

@author: taichi.mitsuhashi
"""

# FASMBCEA_EUR
# Central Bank Survey, Currency in Circulation (Euro Area-Wide-Residency), Euros
...
def get_dict_of_country(country_code):
    ...
    col_df = []
    temp_url = api_url+"IFS/M."+country_code+".FASMBCEA_EUR"
    #print(temp_url)
    ex=""
    try:
...
for key, val in dic_df_summary.items():
    ...
    df_temp.to_csv(f"CSV_Store\\data_EUR_{country_code}_{str_date}.csv")
    ...
    #    pass
    if country_name!="":
        plt.title(f"Fig of FASMBC_{country_name} : Euro")
    else:
    ...
    plt.savefig(f"Image_Store\\fig_EUR_{country_code}_{str_date}.png")
    plt.show()
